...
func TestGraphResultsParse(t *testing.T) {
	p := parser.NewDefaultJmxParser()
	plan, err := p.Parse("../tests/listeners/test_graph_results.jmx")
	if err != nil {
	...
	for _, rc := range plan.ResultCollectors {
		if rc.Filename == "graph_results.jtl" {
			found = true
			if rc.ErrorLogging {
	...
	if !found {
		t.Fatalf("Graph Results listener not found in plan.ResultCollectors")
	}
}
