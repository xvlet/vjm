...
func TestAssertionResultsParse(t *testing.T) {
	p := parser.NewDefaultJmxParser()
	plan, err := p.Parse("../tests/listeners/test_assertion_results.jmx")
	if err != nil {
	...
	for _, rc := range plan.ResultCollectors {
		if rc.Filename == "assertion_results.jtl" {
			found = true
			if rc.ErrorLogging {
	...
	if !found {
		t.Fatalf("Assertion Results listener not found in plan.ResultCollectors")
	}
}
