...
## 4. 체크리스트 및 검증 방법
- [ ] `vegeta` CLI 바이너리가 없는 환경(예: 순수 Docker 컨테이너)에서 `vjm` 실행 시 정상적으로 부하가 발생하는가?
- [ ] 9000 TPS 이상의 고부하 테스트에서 기존 대비 CPU 사용량이나 병목이 개선되었는가?
- [ ] 실시간 대시보드 및 최종 Attack Report 요약이 기존과 동일하거나 더 정확하게 출력되는가?
- [ ] 생성된 `.bin` 파일이 기존 리포트 생성 로직과 완벽히 호환되는가?
